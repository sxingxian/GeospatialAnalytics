...
The imported dataset has a projected CRS of SVY21, which needs to translate it to an international code of 3414.
...
Sometimes when the `tmap()` is executed, the polygon will encounter some invalidity. In order to resolve this issue, add in `tmap_options(check.and.fix=TRUE)` to resolve this issue. Place the `tmap_options()` after the `tm_shape()` to fix the polygon. However, it is ideal to fix the polygon data.
...
Take note not to use the simple feature file, condo_resale.sf, as it contains spatial information.
...
`olsrr()` will provide a neater output. Variables (by looking at the *SIG* column) that are smaller than alpha value of 0.05 will not be rejected as they are statistically significant. Any 1 unit increase in square meter will increase by 12,777.523. When the property increases in age, 1 unit increase in age, the price of the property will drop by 24,687.739. For Boolean fields, such as *freehold*, if it is TRUE, then the *BETA* value will be applied. In this case, when if the property is freehold, the property will increase by 350,599.812.

When it is geographical weighted, also take note of the sign in the *BETA* column in table, Parameter Estimates.

### Preparing Publication Quality Table: gtsummary method

The [**gtsummary**](https://www.danieldsjoberg.com/gtsummary/) package provides an elegant and flexible way to create publication-ready summary tables in R. This package will help generate a neater report in table format. This is an extension of the **gt** package.
...
CV is cross validation method, which can also be replaced with AIC approach. It will keep generating out the CV score and stop at the best score.
...
Comparing the AICc (geographical weighted), the adaptive is slightly smaller at 42967.14.

By default the *longlat* is set to TRUE, it will automatically convert to projected coordinate. If it is set to FALSE, the data is already in its decimal degree. If it is not set to FALSE when there's such data, then it will throw error.
...
Using tm_view(set.zoom.limits=) will restrict the maximum zoom view. When there's two tmap shown side by side, set sync = TRUE within the tmap_arrange() function.
