...
Using st_read() is a sf function of sf package to import the relevant shapefile into R.

**%\>%** is a function to "glue" codes together.
...
Multiply by 1000 instead 100 as we are not calculating percentage. In this case, we are making it to per 1000 households. Using `mutate` to create a mathematical function.
...
`left_join` will automatically find the common column name to join the dataset together. If there isn't any common column name, then use the *by* attribute of `left_join` function. When joining, we need to retain the geometric properties from the shan_sf dataframe.
...
`ict_derived[,12:17]` using this method refers to using the columns 12 to 17.

The correlation plot above shows that COMPUTER_PR and INTERNET_PR are highly correlated. This suggest that only one of them should be used in the cluster analysis instead of both.

When the ellipse bottom is facing towards the right, it is negative correlated, if otherwise, it is positive correlated.
...
Using the `select` from dply R function to select the columns that you want to use it in.
...
Using `cex` to scale down to 60%. Its value ranges from 0.1 to 1. This is important to ensure that the axis can be seen clearly.
...
In R, we will use *function()* to execute loops.
...
The following code chunk uses [*as_Spatial()*](https://r-spatial.github.io/sf/reference/coerce-methods.html) of **sf** package to convert *shan_sf* into a SpatialPolygonDataFrame called *shan_sp*. We will need to convert sf to sp to get spatial polygons dataframe. After converting to **sp** the geometry attribute will longer be there.
...
cr <- choicealpha(proxmat, distmat, 
                  range.alpha = seq(0, 1, 0.1), K=6, graph = TRUE)
```

`choicealpha()` is a function to balance up the homogeneity. It will compute at different range, using the cut-off attribute space or spatial space.

The black colored line (D0, proxmat) indicates attribute spaces, while the red colored line (D1, distmat) indicates geographical space.

Changing the seq(0,1,0.1) to seq(0,1,0.2) will change the spacing from 0.1 to 0.2, with range from 0 to 1.

Note that the value **K** is capital letter instead of small letter.
...
Join back the group list with shan_sf polygon feature data frame by using the code chun below.
